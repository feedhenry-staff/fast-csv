...
extended(ParserStream).extend({
    ...
    _parse: function _parseLine(data, hasMoreData) {
        ...
        try {
            ...
            if (!this._parsedHeaders) {
                ...
                //allow transform headers to lowsercase or uppercase
                var transformcase = false;
                var headercase = null;
                if(headers === "lower" || headers === "upper"){
                    transformcase = true;
                    headercase = headers;
                    headers = true;
                }
                ...
                if (extended.isArray(headers)) {
                    ...
                    this.__transform = function (data, index) {
                        ...
                        if (data.length > headersLength) {
                            if(self.__errorTransform){
                                data = self.__errorTransform(data.join(self.delimiter), self.delimiter);
                            } else {
                        ...
                        while (++i < headersLength) {
                            ...
                            if(transformcase){
                                headervalue = headercase === "lower"? headervalue.toLowerCase() : headervalue.toUpperCase();
                            }
                            ret[headervalue] = isUndefined(val) ? '' : val;
                        }
    ...
    errorTransform: function(cb){
        if (!extended.isFunction(cb)) {
            throw new TypeError("fast-csv.Parser#errorTransform requires a function");
        }
        this.__errorTransform = cb;
        return this;
    }
